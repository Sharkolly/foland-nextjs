...
import { useQueryProfileFunction } from "@/Components/Hooks/useQuery";
...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setUser(data);
    const rawDate = user?.userProfile?.createdAt
    ? new Date(user.userProfile.createdAt)
    : new Date();
    
    setFormattedDate(rawDate.toDateString());
    let completion = 0;
  ...
  }, [data, user?.userProfile.createdAt, user?.userProfile?.email, user?.userProfile?.firstName, user?.userProfile?.lastName, user?.userProfile?.phoneNumber, user?.userProfile?.role]);
    ...
    <div className="pt-2 pb-7 mx-auto space-y-6 max-md:p-0">
      ...
      <div className={`${theme ? 'bg-navy-blue' : 'bg-white'} px-10 pt-6 pb-10 max-md:px-7 rounded-lg shadow-md items-start flex flex-col lg:flex-row gap-7 max-md:gap-0`}>
        ...
        <div className="flex-1 space-y-3 max-md:space-y-6  text-left">
          ...
          <p className="text-gray-600">Date Joined: {formattedDate}</p>
        ...
        <ul className="flex flex-col gap-4">
          {!user?.userSavedProperty ? (
            ...
            user?.userSavedProperty?.map(({ property }, index) => (
              <Link key={index} href={`/properties/${property._id}`}>
                <li className="border p-2 rounded">
